---
title: Introduction
description: Open-source, self-hosted checkout. Stripe-compatible API. Powered by the Open Payments protocol.
---

OpenCheckout is an open-source, self-hosted checkout system for accepting payments on the web. It provides a hosted checkout page and a Stripe-compatible API that you deploy on your own infrastructure. Your customers pay using their [Open Payments](https://openpayments.dev) wallet address — a URL-based identifier for their bank account, digital wallet, or mobile money account.
...
| Aspect | OpenCheckout | Stripe / PayPal / Paystack |
|---|---|---|
...
| Per-transaction fees | None | 2.9% + $0.30 typical |
...
| API compatibility | Stripe-compatible sessions API | Stripe's own API |
...
- **Single-command deployment** — `docker compose up` with SQLite included
