...
echo "MAINTENANCE_SECRET=$(openssl rand -hex 32)" >> .env

# Build without sending local env files or private keys into the image
docker compose build

# Create the merchant inside the same Docker volume used by the service.
# At the private-key prompt, enter /run/secrets/merchant-private.pem
...
| `CORS_ALLOWED_ORIGINS` | No | Comma-separated browser origins allowed to call `/api/checkout`. Leave empty for server-to-server access only. |
| `MAINTENANCE_SECRET` | Recommended | Enables authenticated session expiry, idempotency cleanup, and durable webhook replay. |

Run maintenance on a schedule, for example every five minutes:
...
The `setup` profile uses that same volume. Do not run `npm run setup` on the host and then start the default named-volume deployment: the host database is a different file and the container would not contain that merchant.
...
```bash
# Stop writes, copy the database from the named volume, then restart.
...
OpenCheckout uses [Drizzle ORM](https://orm.drizzle.team), but the current schema and connection code are SQLite-specific. PostgreSQL or MySQL would require a deliberate schema migration, a new adapter, concurrency testing, and an operational rollout; it is not a configuration-only switch.
...
The container and `npm start` run the schema bootstrapper before serving traffic. This is not a substitute for release-specific migrations: read the release notes, back up the database, and run the full health and checkout smoke tests after rebuilding.
