...
Enter your API key (the `sk_...` value from the setup wizard or from your key management page) and click **Sign In**. The server validates it and sets an eight-hour, encrypted `HttpOnly`, `Secure`, `SameSite=Strict` dashboard session cookie. Client-side JavaScript cannot read the credential.
...
| **preparing** | OpenCheckout is preparing the incoming payment, quote, and approval grant. |
| **awaiting approval** | The payment is ready to resume at the customer's wallet provider. |
| **processing** | Approval succeeded and final completion is being confirmed. A second payment must not be submitted. |
...
| **canceled** | The merchant canceled the session through the API. Explicit expiry uses `expired`. |
...
The active tab is highlighted with a black background. The **Log out** button on the right ends your dashboard session by clearing the encrypted session cookie.
